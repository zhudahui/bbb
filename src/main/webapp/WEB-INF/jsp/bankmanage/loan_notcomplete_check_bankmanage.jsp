...
<!--头部导航栏        -->
		...
		<div>
			...
			<div id="menu">
				<ul id="Huifold1" class="Huifold">
					...
					<li class="item">
						<h4>贷款<b>+</b></h4>
						<div class="info">
							<a href="${pageContext.request.contextPath}/creditrequest/showNotCompleteCreditRequest_bankmanage?bm_id=<%=session.getAttribute("bm_id")%>" class="active">未审批</a>
							<br>
					...
					<div class="row cl">
						<div class="col-xs-8 col-sm-6 col-xs-offset-4 col-sm-offset-3">
							<input class="btn btn-primary radius" type="button"
								   value="&nbsp;&nbsp;通过&nbsp;&nbsp;"
								   style="background-color: #68838B;" onclick="audit(4)">
							<input class="btn btn-danger radius" type="button"
								    value="&nbsp;&nbsp;拒绝&nbsp;&nbsp;" onclick="audit(5)">
						</div>
					</div>
					<!--调整布局  -->
			...
			<script>
				//表单验证
				function pass_check(){  
					var bankidea = document.getElementById("bankidea").value;
					//alert(schoolidea);
    				if(bankidea == null || bankidea == ("")){  
    					modalalertdemo();  
    				} else{
    					modaldemo_pass();
    				}
				...
				function pass(){
					...
					var bankidea = document.getElementById("bankidea").value;
					window.location.href = "${pageContext.request.contextPath}/creditrequest/pass_bankmanage?r_id="+r_id+"&bankidea="+bankidea;
				}
				function fail(){
					var r_id = document.getElementById("r_id").value;
					var bankidea = document.getElementById("bankidea").value;
					window.location.href = "${pageContext.request.contextPath}/creditrequest/fail_bankmanage?r_id="+r_id+"&bankidea="+bankidea;
				}
			</script>
			...
			<script>
				...
				function audit(s) {
					$.post("${pageContext.request.contextPath}/creditrequest/audit", {
						...
						window.top.location.href = '${pageContext.request.contextPath}/creditrequest/showNotCompleteCreditRequest_bankmanage?bm_id=<%=session.getAttribute("bm_id")%>';
                    }, 'json')
                }
			</script>
