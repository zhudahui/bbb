...
<body>
	...
	<div>
		...
		<div id="main"
			...
			<!--面包屑导航  -->
			<nav class="breadcrumb">贷款<span class="c-666 en">&gt;</span>
			<span class="c-666">查询申请</span></nav>

			<!--搜索框-->
			<br />
			<div class="clearfix">
				<form method="get" action="${pageContext.request.contextPath}/creditrequest/search_admin">
			...
			<table class="table table-border table-bordered table-hover">
				...
				<%-- <tbody>
					<c:forEach items="${all}" var="request" begin="0" step="1">
						<tr class="text-c">
							<td>${request.getR_id() }</td>
							<td>${request.getS_id() }</td>
				...
				<%
					...
					if (all != null) {
						...
						for (int i = 0; i < all.size(); i++) {
							...
							String b_id = creditrequest.getB_id();
				...
				<tr class="text-c">
					...
					<%-- <td><%=flag%></td> --%>
					<td><a
						href="${pageContext.request.contextPath}/creditrequest/showCreditRequestDetail_admin?r_id=<%=creditrequest.getR_id()%>&s_id=<%=creditrequest.getS_id()%>"
					...
					<td><input class="btn btn-danger radius size-MINI"
							type="button" onClick="delete_check('<%=creditrequest.getR_id()%>')" value="删除"></td>
				</tr>
				</tbody>
				<%
					}
