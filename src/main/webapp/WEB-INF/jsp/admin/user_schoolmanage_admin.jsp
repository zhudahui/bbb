...
<!--头部导航栏        -->
		...
		<div>
			...
			<div id="main" style="margin-left:16.67%;padding:1px 16px;height:1000px;">
				<!--面包屑导航  -->
				<nav class="breadcrumb">用户<span class="c-666 en">&gt;</span><span class="c-666">学校用户</span></nav>

				<!--搜索框-->
				<br />
				<br />
				<div class="clearfix">
					<form method="get" action="${pageContext.request.contextPath}/schoolmanage/search_admin">
						...
						<!--添加用户按钮-->
						<a class="btn btn-primary radius" href="${pageContext.request.contextPath}/school/showAllSchool">添加用户</a>
					</form>
				</div>
				...
				<table id="student" class="table table-border table-bordered table-hover">
					...
					<tbody>
						<c:forEach items="${all}" var="schoolmanage" begin="0" step="1">
						<tr class="text-c">
							<td>${schoolmanage.getSm_id() }</td>
							<td>${schoolmanage.getSm_name() }</td>
							<td>${schoolmanage.getSm_school() }</td>
							<td><a class="btn btn-primary radius size-MINI" href="${pageContext.request.contextPath}/schoolmanage/showSchoolManageDetail_admin?sm_id=${schoolmanage.getSm_id() }">查看</a></td>
							<td><a class="btn btn-primary radius size-MINI" href="${pageContext.request.contextPath}/schoolmanage/showSchoolManageDetail_admin?sm_id=${schoolmanage.getSm_id() }">修改</a></td>
							<td><input class="btn btn-danger radius size-MINI"
							type="button" onClick="delete_check('${schoolmanage.getSm_id() }')" value="删除"></td>
						</tr>
						</c:forEach>
					</tbody>
				</table>
		...
		<script>
			var id = new String();
			function delete_check(sm_id) {
				id = sm_id;
				$("#modal-demo-delete").modal("show");
			}
			function delete_yes(){
				window.location.href = "${pageContext.request.contextPath}/schoolmanage/delete_admin?sm_id="+id;
			}
		</script>
